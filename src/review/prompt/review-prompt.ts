...
export function buildSystemPrompt(): string {
  return [
    ...
    '7. A `duplicate` evidence item is a measured STRUCTURAL similarity, not a verdict.',
    '   Two functions can be structurally alike and still be right to keep apart. Report',
    '   one only when consolidating them would actually be an improvement, and say what',
    '   the cost of the duplication is — a fix applied to one copy and not the other, a',
    '   rule that now has two homes. Never report it as a defect on similarity alone.',
    '',
    'SEVERITY',
    ...
    '  duplicate-logic         — the change re-implements logic that already exists',
    '',
    'OUTPUT',
